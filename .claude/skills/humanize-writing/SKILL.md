...
description: |
  ...
  Run this as the final pass on human-facing pdotj.co copy (Reels, carousels, emails,
  WhatsApp, landing pages) after brand-voice/de-slop drafting, per CLAUDE.md.
license: MIT
metadata:
  version: "3.0.0"
...
<!--
Vendored verbatim from blader/humanizer (MIT License, Copyright (c) 2025 Siqi
Chen — see LICENSE in this folder), renamed to `humanize-writing` to match
this repo's CLAUDE.md rule. Content is unmodified from the upstream skill.
-->
...
Per this repo's `CLAUDE.md`, run this skill as the final pass on any
human-facing document (Reels scripts, carousels, captions, emails, WhatsApp
broadcasts, landing page copy) after brand-voice / de-slop drafting is done —
not as a replacement for it. It fixes structural and stylistic AI tells; it
does not check facts, strategy, or whether the copy matches Prathamesh's
voice.
